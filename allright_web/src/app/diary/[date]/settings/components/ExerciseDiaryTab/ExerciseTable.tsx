    ...
    return (
      <div className="max-w-[930px]">
        <div className="flex justify-between items-center mb-4">
          <label className="text-base font-medium">운동기록</label>
        ...
        <div className="border border-white/10 rounded-lg overflow-hidden">
          <div className="overflow-x-auto hide-scrollbar">
            ...
            {exercises.map((exercise) => (
              <div key={exercise.id} className="flex border-b border-white/10 last:border-b-0">
                <div className="w-32 px-3 py-2 flex-shrink-0 border-r border-white/10">
                  <input
                    type="text"
                    value={exercise.name}
                    onChange={(e) => onUpdateExerciseName(exercise.id, e.target.value)}
                    placeholder="운동명 입력"
                    className="w-full px-2 py-1 text-xs border border-white/10 rounded placeholder-gray-500 focus:border-[#16a34a] focus:outline-none"
                  />
                </div>

                {Array.from({ length: maxSets }, (_, i) => i).map((index) => (
                  <div key={index} className="w-20 px-1 py-2 flex-shrink-0 border-r border-white/10">
                    <div className="flex gap-1">
                      <input
                        type="text"
                        placeholder="무게"
                        ...
                        className="w-8 px-1 py-1 text-xs border border-white/10 rounded placeholder-gray-500 focus:border-[#16a34a] focus:outline-none"
                      />
                      <input
                        type="text"
                        placeholder="횟수"
