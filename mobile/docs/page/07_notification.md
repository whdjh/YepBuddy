...
설정 화면
  ...
  └─ 프로틴 세일 알림 토글 OFF
      └─ enabled false 저장 및 예약 취소
  ├─ 현재 위치를 운동 장소로 등록/재등록
  │   ├─ foreground 위치 권한 확인/요청 가능
  │   └─ 정확도 20m 이내인 현재 위치만 단일 장소로 저장
  ├─ 운동 장소 알림 토글 ON
  │   ├─ OS 알림 권한 확인/요청 가능
  │   ├─ foreground 위치 권한 확인/요청 가능
  │   ├─ background 위치 권한 확인/요청 가능
  │   ├─ 권한 허용 → 등록 장소의 50m Enter geofence 등록 및 enabled true 저장
...
운동 종료
  └─ 로컬 세션/헬스킷/캘린더 저장 흐름
      ...
      └─ 등록 장소가 있으면 완료 시각부터 장소 알림 cooldown 12시간 갱신
...
| 설정 화면 | `/settings` | 단일 운동 장소 등록/교체/해제와 geofence ON/OFF 제어 |
...
| `workout-place-arrival` | `entities/workout-session` | 사용자가 등록한 운동 장소 도착 알림 |
...
- 사용자가 설정 화면에서 현재 위치를 운동 장소로 직접 등록한다.
- 등록 가능한 장소는 1곳이며, 등록과 재등록 모두 위치 정확도가 `20m` 이내여야 한다.
- 등록 위치를 중심으로 앱을 깨우기 위한 반경 `50m` Enter-only geofence 1개를 등록한다.
- 완료 세션 위치 누적, 120m 장소 병합, 2회 방문 후보 판정은 사용하지 않는다.

권한 규칙:

- 위치 등록/재등록은 foreground 위치 권한만 요청할 수 있다. `운동 장소 알림` ON은 알림 권한, foreground 위치 권한, background 위치 권한을 요청할 수 있다.
- 앱 시작과 자동 재동기화에서는 현재 권한 상태만 확인한다.
- 자동 동기화에서 권한이 꺼져 있으면 enabled 값은 유지하고 geofence 등록 중지와 동기화 상태 저장만 수행한다.
- 토글 OFF는 등록 위치를 유지하고 geofence만 중지한다. 등록 해제는 위치를 삭제하고 OFF로 전환한다.

알림 규칙:

- OS geofence Enter 이벤트는 알림 후보 신호다. 이벤트만으로 바로 알림을 보내지 않는다.
- Enter 이벤트를 받으면 권한 프롬프트 없이 현재 위치를 1회 확인한다.
- 현재 위치 정확도와 등록 위치까지의 거리가 모두 `20m` 이내일 때만 운동 시작 제안 알림을 보낸다.
- `countdown`, `recording`, `paused` 상태에서는 알림을 보내지 않는다.
- 등록/재등록, 운동 완료, 마지막 도착 알림 중 가장 최근 시각부터 12시간 cooldown을 적용한다.
- Exit 이벤트와 운동 종료 누락 리마인더는 사용하지 않는다.
- 도착 알림 data에는 `type: "workout-place-arrival"`와 고정 `placeId`만 포함한다.
...
OS의 50m Enter 이벤트는 앱을 깨우는 후보 신호다. 실제 알림 여부는 현재 위치를 한 번 다시 조회해 거리와 정확도가 모두 20m 이내인지 확인하고, 현재 운동 상태와 12시간 cooldown을 적용해 결정한다.
...
| --- | --- | --- |
| 운동 시작 제안 | 사용자가 등록한 단일 운동 장소의 20m 안에 도착함 | 운동일지에서 운동 시작 확인 Alert 표시 |
...
| 등록 위치에서 `20m`보다 멂 | 알림 없음 |
| 운동 카운트다운, 기록 또는 일시정지 중임 | 알림 없음 |
| 등록/재등록 후 12시간이 지나지 않음 | 알림 없음 |
| 운동 완료 후 12시간이 지나지 않음 | 알림 없음 |
| 마지막 도착 알림 후 12시간이 지나지 않음 | 알림 없음 |
...
| 조건 | 기준 |
| --- | --- |
| 등록 장소 | 사용자가 설정에서 직접 등록한 단일 위치가 있음 |
| geofence 이벤트 | 반경 `50m` Enter 이벤트를 받음 |
| 재검증 위치 | 현재 위치 정확도 `20m` 이내, 등록 위치까지 거리 `20m` 이내 |
| 운동 상태 | `idle` 또는 `completed` |
| 재알림 제한 | 등록/재등록, 운동 완료, 마지막 알림 중 가장 최근 시각부터 12시간 경과 |

#### 5.8.4 12시간 cooldown

등록, 운동 완료, 알림 발송은 하나의 도착 알림 cooldown을 공유한다.

- 장소 등록 또는 재등록에 성공하면 해당 시각부터 12시간 동안 알림을 차단한다.
- 운동을 완료하면 완료 시각부터 12시간 동안 알림을 차단한다.
- 도착 알림 예약이 성공하면 발송 시각부터 12시간 동안 알림을 차단한다.
- 세 시각 중 가장 최근 값을 기준으로 한다.
...
- `yb:workout-place-reminder:confirmed-place`
  - 값: 단일 등록 장소 좌표와 등록 시각(JSON)
- `yb:workout-place-reminder:cooldown-started-at`
  - 값: 등록/재등록, 운동 완료, 알림 성공 중 가장 최근 시각
