...
앱 실행
  └─ 루트 레이아웃 초기화
      ...
      └─ 저장된 운동 리마인더 활성 상태 확인
          └─ 활성 상태면 현재 권한만 확인하고 다음 22:00 알림 재예약
...
설정 화면
  ...
  │   ├─ 권한 허용 → 다음 22:00 알림 예약 및 enabled true 저장
...
- 현재 시각이 22:00 이전이면 오늘 22:00에 예약한다.
...
4. 자동 동기화 경로는 OS 알림 권한 요청을 시작하지 않는다.
5. 장소 도착 알림은 OS geofence 정책에 따라 지연되거나 전달되지 않을 수 있다.
