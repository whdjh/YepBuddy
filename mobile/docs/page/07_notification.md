...
설정 화면
  ...
  │   ├─ 권한 허용 → 최근 장소 최대 10개의 150m/50m Enter geofence 등록 및 enabled true 저장
...
- 최근 방문 순 최대 10개 장소를 앱을 깨우기 위한 반경 `150m` 외곽과 `50m` 내부 Enter-only geofence로 등록한다.
...
- 외곽 또는 내부 Enter 이벤트를 받으면 최대 5분간 background 위치를 반복 확인한다.
- 현재 위치 정확도와 등록 위치까지의 거리가 모두 `20m` 이내인 상태를 2회 연속 확인할 때만 운동 시작 제안 알림을 보낸다.
- 알림 발송, 시간 만료 또는 정책 차단 시 background 위치 추적을 중지한다.
