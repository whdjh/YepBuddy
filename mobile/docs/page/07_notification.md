...
- Exit 이벤트를 받거나 앱이 active로 돌아왔을 때 진행 중 운동이 있으면 권한 프롬프트 없이 현재 위치를 1회 확인하고, 위치 정책을 통과한 경우에만 운동 종료 누락 리마인더를 보낸다.
- 같은 운동 종료 리마인더는 12시간 cooldown, 전체 종료 리마인더는 30분 cooldown을 적용한다.
- 도착 알림 data에는 `type: "workout-place-arrival"`과 `placeId`만 포함한다.
- 종료 리마인더 data에는 `type: "workout-place-exit-reminder"`, `placeId`, `sessionId`를 포함한다.
