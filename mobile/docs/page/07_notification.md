...
- 새 장소 표본이 기존 중심점 `30m` 이내면 같은 장소로 병합한다.
...
- 단발 확인 시점에 `20m` 밖이면 해당 진입에서는 알림이 없고, 지속 위치 추적이나 재시도는 하지 않는다.
...
OS의 50m Enter 이벤트는 앱을 깨우는 후보 신호다. 이벤트 후 현재 위치를 한 번 조회해 거리와 정확도가 모두 20m 이내인지 확인하고, 현재 운동 상태와 당일 알림 제한을 적용해 결정한다.
...
| -------------- | ------------------------------------------------ | -------------------------------------- |
| 운동 시작 제안 | 자동 학습된 운동 장소 중 한 곳의 20m 안에 도착함 | 운동일지에서 운동 시작 확인 Alert 표시 |
