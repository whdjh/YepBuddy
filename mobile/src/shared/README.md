...
| `formatValue` | 단위까지 포함한 값 문자열 반환. 최고·평균·최저 모두 같은 formatter 사용 |
| `color` | token hook으로 해석한 실제 색상 문자열 |
| `height` | 차트 캔버스 높이. 카드, 통계, 양 끝 라벨 높이는 제외 |
| `xDomain` | 선택적 `[시작, 종료]`. 유한한 값이며 시작 < 종료. 지정 시 구간 밖 point 제외, 실제 X 간격 보존 |
| `area`, `guides`, `showAverage` | 각각 영역, 가이드, 평균 문구 표시. 기본값 `false` |
...
- `showAverage=true`이면 동일 값이나 가이드 겹침 여부와 관계없이 평균 문구를 표시합니다. 평균 가이드는 최저·최고에서 각각 범위의 15%보다 멀 때 표시하며, 최고=최저 가이드는 한 번만 그립니다.
