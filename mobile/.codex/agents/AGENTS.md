...
- 큰 작업은 먼저 범위를 나누고, 가능한 경우 아래 역할 템플릿을 서브에이전트/별도 Codex 세션에 디스패치한다.
- 단순 질의, 명확한 한 줄 수정, 오타 수정, 포맷 변경처럼 병렬화 비용이 더 큰 작업은 메인 Codex가 직접 처리할 수 있다.
...
아래 역할 파일은 작업을 나눠서 생각하거나 별도 Codex 세션/서브에이전트에 전달할 때 쓰는 프롬프트 템플릿이다. 기능 구현, 버그 수정, 코드 리뷰, 테스트 실패 분석, 화면 흐름 변경, FSD 경계 확인, 멀티파일 수정은 가능한 한 이 역할들로 나누어 디스패치한다.
